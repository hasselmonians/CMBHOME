function [rate_map, xdim, ydim, occupancy, no_occupancy, spk_locs, pos_locs] = sRateMap(self, varargin)
...
%   spk_locs        bin which each spike falls in.
%   pos_locs        bin which each position falls in
%
% OPTIONAL PARAMETERS
%   
...
% jrc update 130114_1045 - add pos_locs
[occupancy, xdim, ydim, pos_locs] = self.sOccupancy(xdim, ydim, continuize_epochs, binside);
...
if ~iscell(spk_x)
    % jrc update 130114_1043 - add spk_locs
    % spikes = hist3([spk_x, spk_y], 'Edges', {xdim, ydim});
    %%
    [spikes,~,~,temp] = histcn([spk_x spk_y],[xdim max(xdim+binside)],[ydim max(ydim+binside)]);
    
    spk_locs = zeros(size(temp,1),1);
    spk_locs(~any(temp==0,2)) = sub2ind(size(spikes'),temp(~any(temp==0,2),2),temp(~any(temp==0,2),1));
    ...
    spk_locs = cell(size(occupancy,3),1);

    for i = 1:size(occupancy,3)

        %         spikes = hist3([spk_x{i}, spk_y{i}], 'Edges', {xdim, ydim});
        [spikes,~,~,spk_locs{i}] = histcn([spk_x{i} spk_y{i}],[xdim max(xdim+binside)],[ydim max(ydim+binside)]);
        spk_locs{i} = sub2ind(size(spikes),spk_locs{i}(:,1),spk_locs{i}(:,2));
