        ...
        function self = AppendKalmanVel(self)
            ...
            function self = get_speed(self)
                if ~isempty(self.b_vel)
                    ...
                    if ~isnan(self.b_x(1)) && ~isnan(self.b_x(end))
                        ...
                        %determine # of NaNs at beginning
                        if isnan(self.b_x(1))
                            ind = 1;
                            startnancount = 0;
                            while isnan(self.b_x(ind)) && ind < length(self.b_x)
                                startnancount = startnancount + 1;
                                ind = ind + 1;
                            end
                        end
                        
                        %determine # of NaNs at end
                        if isnan(self.b_x(end))
                            ind = length(self.b_x);
                            endnancount = 0;
                            while isnan(self.b_x(ind)) && ind >= 1
                                endnancount = endnancount + 1;
                                ind = ind - 1;
                            end
                        end
                        
                        %trim edges of needed data
                        if exist('startnancount','var') && exist('endnancount','var')
                            raw_speed = self.svel(startnancount+1:end-endnancount);
                            xvec = self.b_x(startnancount+1:end-endnancount);
                            yvec = self.b_x(startnancount+1:end-endnancount);
                            tvec = self.b_ts(startnancount+1:end-endnancount);
                            x = self.x(startnancount+1:end-endnancount);
                            y = self.y(startnancount+1:end-endnancount);
                            ts = self.ts(startnancount+1:end-endnancount);
                        elseif exist('startnancount','var')
                            raw_speed = self.svel(startnancount+1:end);
                            xvec = self.b_x(startnancount+1:end);
                            yvec = self.b_x(startnancount+1:end);
                            tvec = self.b_ts(startnancount+1:end);
                            x = self.x(startnancount+1:end);
                            y = self.y(startnancount+1:end);
                            ts = self.ts(startnancount+1:end);
                        elseif exist('endnancount','var')
                            raw_speed = self.svel(1:end-endnancount);
                            xvec = self.b_x(1:end-endnancount);
                            yvec = self.b_x(1:end-endnancount);
                            tvec = self.b_ts(1:end-endnancount);
                            x = self.x(1:end-endnancount);
                            y = self.y(1:end-endnancount);
                            ts = self.ts(1:end-endnancount);    
                        else
                            fpritnf('You screwed up\n');
                        end
                        
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %original get_speed code with changes for nans%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        [t,~,~,vx,vy] = self.KalmanVel(xvec,yvec,tvec, 2); % quadratic
                        [~, inds] = ismember(t, tvec); % all indices in self.b_ts for which we have a kalman estimate
                        speed = zeros(size(tvec));
                        speed(inds) = sqrt(vx.^2 + vy.^2); % pixels/sec  
                        
                        x_rev = flipud(x);
                        y_rev = flipud(y);
                        [~,~,~,vx_rev,vy_rev] = self.KalmanVel(x_rev,y_rev,ts,2);
                        speed_rev = sqrt(vx_rev.^2 + vy_rev.^2);
                        speed_rev = flipud(speed_rev); % Kalman filtered speed of reversed movement 
                        ...
                        smooth_factor = 15; % smooth factor, 1 is 1/fs_video seconds
                        smooth_x = smooth(x,smooth_factor); %smooth tracking data
                        smooth_y = smooth(y,smooth_factor);
                        smooth_speed = sqrt(diff(smooth_x).^2 + diff(smooth_y).^2)*self.fs_video;
                        combined_speed = sqrt(corrected_speed(2:end).*smooth_speed);
                        velocity = [corrected_speed(1);combined_speed];
                        
                        self.b_vel = [nan(startnancount,1); velocity; nan(endnancount,1)];
                    end                        
                end
            end
