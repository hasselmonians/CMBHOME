function [status,interval,index,bounds] = InIntervals2(values,intervals,varargin)
...
for i = 1:n,
    ...
    % Get values
    more = CMBHOME.Utils.FindInInterval(values,[from to],previous);
%    more = FindInInterval(values,[from to],previous);       % BSC 170127 need to call FindInInterval.mexa64 instead of .m
    if ~isempty(more),
